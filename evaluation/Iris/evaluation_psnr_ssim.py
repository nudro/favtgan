...
NOTE - THIS VERSION HAS THE PATH SET TO 'IRIS'
"""
...
def ssim(master_array):
    ...
    print(values)
    table = pd.DataFrame(values)
    iris_test_files = pd.read_csv('iris_test.txt', sep=" ", header=None)
    b = iris_test_files[0]
...
def main(fake_dir, real_dir, experiment):
    ...
    # Make sure path is right - this is for Iris
    psnr_table.to_csv('/Users/catherine/Documents/GANs_Research/my_imps/research_models/evaluation/Iris/%s/psnr.csv' % (experiment))
    ...
    # Make sure path is right - this is for Iris
    ssim_table.to_csv('/Users/catherine/Documents/GANs_Research/my_imps/research_models/evaluation/Iris/%s/ssim.csv' % (experiment))
