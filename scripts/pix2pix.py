...
"""
========================================================
# Need to update this for each label experiment

eurecom_adas_d.csv for domain labels on eurecom_adas [7-31]
eurecom_adas_s.csv for sensor labels on eurecom_adas
eurecom_adas_iris_d.csv for domain on eurecom_adas_iris [done]
eurecom_adas_iris_s.csv for sensor labels
...
"""
    Need to call the files of AB concatenated like we have.
    But then go down
"""
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # print("fake_B shape is:", fake_B.size())


        # GAN loss
        # change tensor type to make it work in the d embedding?
        # gen_labels = gen_labels.type(torch.FloatTensor)

        print("input_shape that will be an arg into discriminator is:", input_shape)

        #print("====discriminator(fake_B, real_A, gen_labels)=====\n")
        # pred_fake, pred_label = discriminator(fake_B, real_A, gen_labels)

        # pred_fake = discriminator(fake_B, real_A, gen_labels)
        # trying the below while the above ^^ works
        ...
        #print("torch.max(pred_label, -1)[0]:", torch.max(pred_label, -1)[0])
        #print("torch.max(labels, -1).indices:", torch.max(pred_label, -1).indices)
        # print("====generated label is:", gen_labels)
        print("generated label is:", gen_labels.size())
        # print("====real label is:", labels)
        ...
        # criterion(input, target) - how well predicted label matches generated labels
        ...
        # d_pred_real = discriminator(real_imgs, labels)

        # pred_real, real_aux = discriminator(real_B, real_A, labels)
        #pred_real = discriminator(real_B, real_A, labels)

        #print("===starting discriminator(real_B, real_A, labels)======\n")
        pred_real, real_aux = discriminator(real_B, real_A, labels)
        # print("pred_real shape:", pred_real.size())
        # print("real_aux shape:", real_aux.size())

        loss_real = criterion_GAN(pred_real, valid)

        #below is what I added for the label loss
        #above ^^ I added the label output 'real_aux'
        ...
        # gen_labels i think so - this is what cgan does:
        # d_pred_fake = discriminator(gen_imgs.detach(), gen_labels)
        #pred_fake = discriminator(fake_B.detach(), real_A, gen_labels)

        #print("===starting discriminator(fake_B.detach(), real_A, gen_labels)======\n")
