...
#criterion_VGG = torch.nn.MSELoss(reduction='sum') #Euclidean distance (y^ - y).pow(2).sum()
criterion_cx = cl.ContextualLoss(use_vgg=True, vgg_layer='relu5_4')
...
def sample_images(batches_done):
    ...
    print("sample images labels", labels)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # Pixel-wise loss
        # Between the SR image and the real B image
        loss_pixel = criterion_pixelwise(fake_B, real_B)
        print("loss_pixel:", loss_pixel)


        # contextual loss with VGG
        # comparing with VGG features
        # if `use_vgg` is set, VGG model will be created inside of the criterion

        loss_cx = criterion_cx(real_B, fake_B)
        print("context loss:", loss_cx)
        #loss_cx = F.contextual_loss(real_B, fake_B, band_width=0.1, loss_type='cosine')

        # Total loss
        #loss_G = 0.5 * (loss_GAN + label_loss + lambda_pixel * loss_pixel)
        #loss_G = 0.5 * (loss_GAN + label_loss + loss_VGG)
        loss_G = 0.5*(loss_GAN + label_loss + loss_cx)
        print("loss_G:", loss_G)
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [D_real adv: %f, aux: %f, total: %f] [D_fake adv: %f, aux: %f, total: %f][G loss: %f,  adv: %f, aux: %f, CX: %f] ETA: %s"
            % (
                epoch, #%d
                opt.n_epochs, #%d
                i, #%d
