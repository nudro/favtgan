...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [D_real adv: %f, aux: %f, total: %f] [D_fake adv: %f, aux: %f, total: %f][G loss: %f,  adv: %f, aux: %f, LPIPS: %f] ETA: %s"
            % (
                epoch, #%d
                opt.n_epochs, #%d
                i, #%d
